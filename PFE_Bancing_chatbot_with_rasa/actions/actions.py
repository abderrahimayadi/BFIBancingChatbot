...
class ActionTransferMoney(Action):
    ...
    async def run(
        ...
        for event in reversed(tracker.events):
            if event['event'] == 'user' and 'parse_data' in event and 'intent' in event['parse_data']:
                if event['parse_data']['intent']['name'] == 'transfer_money':
                    intent_name = 'transfer_money'
                    break
                elif event['parse_data']['intent']['name'] == 'transfer_money_fr':
                    intent_name = 'transfer_money_fr'
                    break
        print("******************",intent_name)
        if tracker.get_slot("zz_confirm_form") == "yes":
            amount_of_money = float(tracker.get_slot("amount-of-money"))
            ...
            if intent_name == "transfer_money_fr":
                dispatcher.utter_message(response="utter_transfer_complete_fr")
            else:
            ...
            if intent_name == "transfer_money":
                dispatcher.utter_message(response="utter_transfer_cancelled")
            elif intent_name == "transfer_money_fr":
                dispatcher.utter_message(response="utter_transfer_cancelled_fr")
    ...
    async def run(
        ...
        if account_type == "credit":
            ...
            if credit_card and credit_card.lower() in available_cards:
                ...
                elif (intent_name == "check_balance_fr"):
                    dispatcher.utter_message(
                    response="utter_credit_card_balance_fr",
                    **{
                        "credit_card": credit_card.title(),
                        "credit_card_balance": f"{current_balance:.2f}",
                    },
            ...
            if amount:
                ...
                if (intent_name=="check_balance"):
                    ...
                    dispatcher.utter_message(
                    response="utter_changed_account_balance_fr",
                    init_account_balance=f"{init_account_balance:.2f}",
                    account_balance=f"{account_balance:.2f}",
                )
            else:
                if (intent_name)=="check_balance":
                    ...
                    dispatcher.utter_message(
                    response="utter_account_balance_fr",
                    init_account_balance=f"{account_balance:.2f}",
                )
...
class ActionShowRecipients(Action):
    ...
    async def run(
        ...
        if (intent_name=="check_recipients"):
            dispatcher.utter_message(
            response="utter_recipients",
            formatted_recipients=formatted_recipients,
        )
        elif (intent_name=="check_recipients_fr"):
            dispatcher.utter_message(
            response="utter_recipients_fr",
            formatted_recipients=formatted_recipients,
        )
...
class ActionShowTransferCharge(Action):
    ...
    async def run(
        ...
        """Executes the custom action"""
        if (intent_name )== "ask_transfer_charge":
            dispatcher.utter_message(response="utter_transfer_charge")
        elif (intent_name )== "ask_transfer_charge_fr":
            dispatcher.utter_message(response="utter_transfer_charge_fr")
        events = []
        active_form_name = tracker.active_form.get("name")
        if active_form_name:
            # keep the tracker clean for the predictions with form switch stories
...
class ActionAskTransactionSearchFormConfirm(Action):
    ...
    async def run(
        ...
        for event in reversed(tracker.events):
            if event['event'] == 'user' and 'parse_data' in event and 'intent' in event['parse_data']:
                if event['parse_data']['intent']['name'] == 'search_transactions':
                    intent_name = 'search_transactions'
                    break
                elif event['parse_data']['intent']['name'] == 'search_transactions_fr':
                    intent_name = 'search_transactions_fr'
                    break
        if (intent_name=="search_transactions"):
             if vendor_name:
                vendor_name = f" with {vendor_name}"
        elif (intent_name=="search_transactions_fr"):
             if vendor_name:
                vendor_name = f" avec {vendor_name}"
        else:
            vendor_name = ""
        if search_type == "spend":
            if (intent_name=="search_transactions"):
                ...
                text = (
                    f"Voulez vous recherches les transactions{vendor_name} entre "
                    f"{start_time_formatted} and {end_time_formatted}?"
                )
            ...
            if (intent_name == "search_transactions"):
                ...
                text = (
                    f"Voulez vous recherche les gains dans votre compte entre "
